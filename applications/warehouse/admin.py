...
def modif():
    ...
    array = database.session.query(
        ProductSQLA.name,
        func.sum(ReservedList.wanted),
        func.sum(ReservedList.wanted) - func.sum(ReservedList.inStock)
    ).filter(ProductSQLA.id == ReservedList.productID).group_by(ProductSQLA.name).all()
    ...
    #
    # return jsonify("statistics", str(database.session.query(
    #     ProductSQLA.name,
    #     func.sum(ReservedList.wanted),
    #     func.sum(ReservedList.wanted) - func.sum(ReservedList.inStock)
    # ).filter(ProductSQLA.id == ReservedList.productID).group_by(ProductSQLA.name).all()
    ...
    # return str(database.session.query(
    #     ProductSQLA.name
    # ).all())
